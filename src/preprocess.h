...
enum LID_TYPE{AVIA = 1, VELO16, OUST64, LIVOXRAW, MARSIM, AEVA}; //{1, 2, 3, 4, 5, 6}
...
// clang-format off
POINT_CLOUD_REGISTER_POINT_STRUCT(aeva_ros::Point,
    (float, x, x)
    (float, y, y)
    (float, z, z)
    (float, intensity, intensity)
    ...
    (std::int32_t, time_offset_ns, time_offset_ns)
    (std::uint8_t, line_index, line_index)
)
